...
function checkOpts() {
  ...
  if (!isNaN(parseInt($("#size")[0].value, 10))) {
    $("#size")[0].value = options.cellSize = Math.max(
      parseInt($("#size")[0].value, 10),
      4
    );
  }
...
$(document).ready(function() {
  ...
  (canvas = $("#canvas")[0]), (context = canvas.getContext("2d"));
...
function init(options) {
  ...
  $("#showhide").toggle(!options.hideMenuButton);
  $("#menu").toggle(options.showMenu);
}
